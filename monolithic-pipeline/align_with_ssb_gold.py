...
BASE_DIR   = "cleaned_ocr_txt/gold-ocr"
...
OUTPUT_DIR = "gold_parallel_corpus"
...
def main():
    ...
    for filename in os.listdir(DE_DIR):
        if not filename.startswith("gold_de_") or not filename.endswith(".txt"):
            continue

        year = filename.replace("gold_de_", "").replace(".txt", "")

        lang_dirs = {
            "fr": (FR_DIR, f"gold_fr_{year}.txt"),
            "it": (IT_DIR, f"gold_it_{year}.txt"),
            "rm": (RM_DIR, f"gold_rm_{year}.txt"),
        }
