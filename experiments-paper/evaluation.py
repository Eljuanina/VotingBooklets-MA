...
for sheet_name in sheet_names:
    ...
    if df_sheet.shape[1] < 3:
        print(f"Sheet {sheet_name} skipped (less than 3 columns)")
        continue
    
    gold_series = df_sheet.iloc[:, 0].fillna("").astype(str)
    agent_series = df_sheet.iloc[:, 1].fillna("").astype(str)
    pytesseract_series = df_sheet.iloc[:, 2].fillna("").astype(str)
    
    # Calculate metrics for Agent (column 2)
    agent_metrics = calculate_metrics(gold_series, agent_series, "Agent", sheet_name)
    
    # Calculate metrics for Pytesseract (column 3)
    pytesseract_metrics = calculate_metrics(gold_series, pytesseract_series, "Pytesseract", sheet_name)
